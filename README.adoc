...
`isResponsive` indicates if the action is resolving while `isResponsiveChange` indicates if that state is different than the previous event. For the first check wherein there is no such previous event `isResponsiveChange` is considered to be `true`.

You can think of `isResponsive` as being your health indicator while `isResponsiveChange` as being your alert to when crashing or recovery occurs. `isResponsiveChange` is technically a convenience that you could calculate yourself but seeing as its often needed and that its calculation requires state it seems worthwhile.
...
Stream of events where `isResponsive` is `true` and `isResponsiveChange` is `true`.

##### #downs

Stream of events where `isResponsive` is `false` and `isResponsiveChange` is `true`.

##### #changes

Stream of events where `isResponsiveChange` is `true`.
